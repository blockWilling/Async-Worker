...
/**
 * 对象深度大小计算器
 * Created by blockWilling  on 2022/3/3.
 */
public class ObjSizeCalculator {
    ...
    public static void main(String[] args) throws IllegalAccessException {
        final ObjSizeCalculator ci = new ObjSizeCalculator();
JSONObject string = new JSONObject();
string.put("eventType","queue_default");
        string.put("body",new Snowflake(0,0).nextId());
//        long l = objectSizeCalculatorMain.sizeOf(string);
        Event event=new Event();
        event.setBody(new Snowflake(0,0).nextId());
//        ReceiptBody receiptBody=new ReceiptBody();
//        receiptBody.setBiNo(new Snowflake(0,0).nextId());
//        receiptBody.setStatus("DELIVRD");
//        event.setBody(receiptBody);
        event.setEventType("queue_default");
        ObjectInfo res;
        res = ci.introspect( event );
